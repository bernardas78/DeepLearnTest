...
def initHypersRand():
    ...
    #   layerdims - dimensions of each layer, except input (size L). Last always 10 for digits
    ...
    learning_rates = [1e-5, 3e-5, 1e-4, 3e-4, 1e-3, 3e-3, 1e-2, 3e-2, 1e-2]
    hypers["learning_rate"] = learning_rates[np.random.randint(0,len(learning_rates))]

    L_max = 3
    L = np.random.randint(1, L_max+1) #1 to L_max
    hypers["L"] = L
